...
| [FIX-STALE-HARM-SILENCE](FIX-STALE-HARM-SILENCE/PRD.md) — **a site cleaned up while it is evading an anti-radiation missile is deaf for the rest of the mission.** `cleanUp()` cancels the HARM timers but leaves `harmSilenceID` set, and nothing will ever clear it — the task that would have is the one just removed. `goLive()` refuses while that field is set, so the site is closed to network designation, to autonomy, **and to the last line of defense and `reportContact` added in FEAT-LAST-LINE-OF-DEFENSE**. Reached by any `addSAMSitesByPrefix()` call, which VEAF missions make on respawn. From [issue #3](https://github.com/VEAF/Skynet-IADS/issues/3), in walder's original | ⬜ |
