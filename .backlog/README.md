...
| [CHORE-TEST-COVERAGE-FLOOR](CHORE-TEST-COVERAGE-FLOOR/PRD.md) — **the suite runs in CI, and nobody knows what it covers.** `lua-tests.yml` runs 18 green suites on every pull request and measures nothing: no `luacov`, no report, no threshold. Measured on 2026-09-19 at `4125b52`: **70.22%**, data tables excluded from the denominator — 1 679 lines of 2 391. Two files carry 69% of the gap, the logger at 10.14% (the status lines the `skynet-runtime-debug` skill parses, pinned by nothing) and `abstract-radar-element` at 71.25% (whose missing tests are the 42 legacy ones `CHORE-PROFESSIONALIZE-THE-REPO` ticket 04 has yet to port). A **living lot**: the measurement and the floor are in place (tickets 01 and 02, PR #20 and #21) — CI publishes the figure on every pull request and fails below **70**, a floor that only goes up. What is left is the climb, **70 → 83 → 90**, one ticket at a time; at 90% the two big blocks are both mandatory, so this lot depends on CHORE-PROFESSIONALIZE-THE-REPO ticket 04 finishing the legacy port | 🔄 |
