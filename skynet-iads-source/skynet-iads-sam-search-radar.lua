do
		...
		if self:isExist() then
			...
			for i = 1, #data do
				...
				for j = 1, #subEntries do
					...
					if
						sensorInformation.type == Unit.SensorType.RADAR and sensorInformation["detectionDistanceAir"]
					then
	...
	function SkynetIADSSAMSearchRadar:isRadarWorking()
		...
		return (
			self:isExist() == true
			and (self:getDCSRepresentation():getSensors() ~= nil or self:getDCSRepresentation():getAmmo() ~= nil)
		)
	end
