			...
			for j = 1, #detectedTargets do
				...
				if firstRadar ~= nil and firstRadar:isExist() then
					local distance = SkynetIADSUtils.round(
						SkynetIADSUtils.metersToNM(
							ewRadar:getDistanceInMetersToContact(
								firstRadar:getDCSRepresentation(),
								contact:getPosition().p
							)
						),
						2
					)
		...
		for i = 1, #samSites do
			...
			for j = 1, #detectedTargets do
				...
				if firstRadar ~= nil and firstRadar:isExist() then
					local distance = SkynetIADSUtils.round(
						SkynetIADSUtils.metersToNM(
							samSite:getDistanceInMetersToContact(
								firstRadar:getDCSRepresentation(),
								contact:getPosition().p
							)
						),
