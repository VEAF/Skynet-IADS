...
Run the `env.mission` probe under **Checking the mission itself** in `test/insim/README.md`.
...
**Done — see [`test/insim/README.md`](../../../test/insim/README.md).** It is the living copy;
a duplicate here would go stale the moment the tier changes, which it already had.

It covers the three setup steps, the edit→run loop and what needs a mission restart, both
fixture kinds, the measured add/destroy/explode behaviour the isolation model rests on, the
helper reference, a troubleshooting table, and the `env.mission` probe.
